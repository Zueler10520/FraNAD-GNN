...
# WOE编码
def woe_encoding(graph, trn_idx, n_bins=10, min_samples=0.01, col_index=None):
    """
    WOE编码实现
    ...
    n_bins: 分箱数量
    min_samples: 最小样本比例
    col_index: 需要编码的列索引，默认为None表示所有列
    
    返回:
    feature: 编码后的特征DataFrame
    """
    # 转换数据为DataFrame
    X = pd.DataFrame(graph.ndata['feat'].numpy())
    y = graph.ndata['label'].numpy()
    ...
    # 存储WOE编码结果
    woe_encoded_X = pd.DataFrame()
    
    for col in col_index:
        # 使用等频分箱进行初始分箱
        trn_X[f'{col}_bin'] = pd.qcut(trn_X[col], 
                                     q=n_bins, 
                                     duplicates='drop',
                                     labels=False)
        ...
        # 计算每个分箱的WOE值
        woe_dict = {}
        for bin_val in trn_X[f'{col}_bin'].unique():
            mask = trn_X[f'{col}_bin'] == bin_val
            pos_rate = np.mean(trn_y[mask])
            neg_rate = 1 - pos_rate
            
            # 处理极端情况
            eps = 1e-10
            if pos_rate < eps or neg_rate < eps:
                woe = 0
            else:
        ...
        # 对整个数据集进行分箱
        X[f'{col}_bin'] = pd.qcut(X[col], 
                                 q=n_bins, 
                                 duplicates='drop',
                                 labels=False)
        
        # 转换为WOE值
        woe_values = X[f'{col}_bin'].map(woe_dict).fillna(0)
        
        # 创建独热编码
        for bin_val in range(len(woe_dict)):
            woe_encoded_X[f'{col}_bin_{bin_val}'] = (X[f'{col}_bin'] == bin_val).astype(float) * woe_dict.get(bin_val, 0)
    
    # 合并原始特征和WOE编码特征
    feature = pd.concat([X, woe_encoded_X], axis=1)
    feature = feature.drop(columns=[f'{col}_bin' for col in col_index])
...
def iv_encoding(graph, trn_idx, n_bins=10, col_index=None):
    """
    基于IV值的特征编码
    IV = Σ(好比例-坏比例)*ln(好比例/坏比例)
    """
    X = pd.DataFrame(graph.ndata['feat'].numpy())
    y = graph.ndata['label'].numpy()
    ...
    iv_encoded_X = pd.DataFrame()
    
    for col in col_index:
        # 等频分箱
        X[f'{col}_bin'] = pd.qcut(X[col], q=n_bins, duplicates='drop', labels=False)
        
        # 计算每个分箱的IV值
        iv_dict = {}
        total_pos = np.sum(y[trn_idx] == 1)
        total_neg = np.sum(y[trn_idx] == 0)
        
        for bin_val in X[f'{col}_bin'].unique():
            mask = X.iloc[trn_idx][f'{col}_bin'] == bin_val
            pos_rate = np.sum(y[trn_idx][mask] == 1) / total_pos
            neg_rate = np.sum(y[trn_idx][mask] == 0) / total_neg
            ...
            if pos_rate < eps or neg_rate < eps:
                ...
                iv = (pos_rate - neg_rate) * np.log(pos_rate / neg_rate)
            iv_dict[bin_val] = iv
            
        # 创建IV编码特征
        for bin_val in range(len(iv_dict)):
            iv_encoded_X[f'{col}_bin_{bin_val}'] = (X[f'{col}_bin'] == bin_val).astype(float) * iv_dict.get(bin_val, 0)
    
    feature = pd.concat([X, iv_encoded_X], axis=1)
    feature = feature.drop(columns=[f'{col}_bin' for col in col_index])
